...
def project_scenario_cost(
    ...
    # population_growth_pct from the slider represents ADDITIONAL growth
    # on top of the real historical baseline already in base_pop_factor,
    # not a replacement for it. The previous version replaced the real
    # 48% Census-derived baseline with the raw slider value, which made
    # a small slider value (for example +10%) compute as LESS risk than
    # doing nothing, since 10% < 48%. That was backwards: more population
    # growth should always increase or hold projected cost, never lower it.
    if population_growth_pct is not None:
        additional_factor = (population_growth_pct / 100) * 0.8
        scenario_pop_factor = base_pop_factor + additional_factor
    else:
...
SOCIAL_COST_SOURCES = {
    "Houston": {
        "displacement": "FEMA HMGP displacement multiplier ($2,800/mo x 4.2mo avg), Urban Institute disaster-to-homelessness research",
        "healthcare": "OECD 2025 disaster healthcare cost ratio (2.2% of direct damage), emergency care and chronic condition disruption",
        "wages": "Bureau of Labor Statistics lost work hours x FEMA displacement count (4.8% of direct damage)",
        "homelessness": "Urban Institute: 12% of displaced population becomes homeless without intervention",
    },
    "Los Angeles": {
        "displacement": "FEMA HMGP displacement multiplier ($3,900/mo x 5.8mo avg), LA County rental market data",
        "healthcare": "OECD 2025 disaster healthcare cost ratio (2.3% of direct damage), wildfire respiratory and trauma care",
        "wages": "Bureau of Labor Statistics lost work hours x FEMA displacement count (6.5% of direct damage)",
...
def _get_real_homelessness_cost(city: str, est_displaced: int) -> dict:
    """Uses real HUD PIT count disaster impact data instead of flat estimate."""
    try:
        from utils.homelessness_data import compute_real_homelessness_cost
        return compute_real_homelessness_cost(city, est_displaced)
    except Exception:
        return {}
...
def _compute_social_costs(city: str, direct_damage_bn: float) -> dict:
    ...
    total_social_mn = displacement_cost_mn + healthcare_cost_mn + lost_wages_mn
    ...
    return {
        ...
        **_get_real_homelessness_cost(city, est_displaced),
