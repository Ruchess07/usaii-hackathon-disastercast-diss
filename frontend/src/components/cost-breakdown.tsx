...
export function CostBreakdown({ slug }: { slug: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {infraGaps.length > 0 && (
        <div className="bg-[#FBF3DB] border border-[#A38D2E]/20 rounded-[8px] p-4 space-y-3">
          <div>
            <p className="text-[10px] uppercase tracking-[0.1em] text-[#7A6A1C] font-medium">
              Infrastructure capacity gaps
            </p>
            <p className="text-[11px] text-[#7A6A1C] mt-1 leading-relaxed">
              Capacity % means how much of the planned protection a project actually delivers
              today. 100% means fully built and able to handle the flood risk it was designed
              for. 40% means it only protects against a fraction of that risk — the rest of
              the gap is exposure your city is still carrying.
            </p>
          </div>
          {infraGaps.map((proj) => (
            <div key={proj.project_name} className="border-t border-[#A38D2E]/20 pt-2 first:border-t-0 first:pt-0">
              <div className="flex items-center justify-between text-sm">
                <span className="font-medium text-[#1E293B]">{proj.project_name}</span>
                <span className="text-[#7A6A1C] font-semibold">
                  {proj.capacity_pct}% capacity
                </span>
              </div>
              <p className="text-[11px] text-[#7A6A1C] mt-0.5">{proj.status}</p>
              {proj.description && (
                <p className="text-[11px] text-[#1E293B] mt-1 leading-relaxed">{proj.description}</p>
              )}
            </div>
          ))}
          <p className="text-[10px] text-[#7A6A1C] pt-1">
            Below 50% capacity means the gap between what was planned and what exists is large
            enough that the next disaster of similar size will likely cause similar or worse damage.
          </p>
        </div>
      )}
    </div>
